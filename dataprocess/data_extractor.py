...
def get_span_att(
    ...
    t2_span: dict[int, list[int]] = dict()
    h2_span: dict[int, list[int]] = dict()
...
def get_e2r(
    ...
    """
    e2r: dict[int, list[int]] = {}
    tok_tok: set[tuple[int, int]] = set()
...
def unirel_extractor(
        ...
        dataset: UniRelDataset | UniRelSpanDataset,
    ...
    tail_preds: np.ndarray = predictions.predictions
    ...
    all_metirc_results = calculate_f1(state_dict, 'all')
    print(f"\nall:  {state_dict} \n {calculate_f1(state_dict, 'all')}")
    print(f"\ne2e:  {e2e_state_dict} \n {calculate_f1(e2e_state_dict, 'e2e')}")
    print(f"\nh2r:  {h2r_state_dict} \n {calculate_f1(h2r_state_dict, 'h2r')}")
    print(f"\nt2r:  {t2r_state_dict} \n {calculate_f1(t2r_state_dict, 't2r')}")
    print(
        f"\ne2e_tail:  {e2e_tail_state_dict} \n {calculate_f1(e2e_tail_state_dict, 'e2e_tail')}")
    print(
        f"\ne2e without rel:  {e2e_plain_state_dict} \n {calculate_f1(e2e_plain_state_dict, 'e2e_plain')}")
    logger.info(f"\nall:  {calculate_f1(state_dict, 'all')}")
    logger.info(f"\ne2e:  {calculate_f1(e2e_state_dict, 'e2e')}")
    logger.info(f"\nh2r:  {calculate_f1(h2r_state_dict, 'h2r')}")
    logger.info(f"\nt2r:  {calculate_f1(t2r_state_dict, 't2r')}")
    logger.info(
        f"\ne2e_tail:  {calculate_f1(e2e_tail_state_dict, 'e2e_tail')}")
    logger.info(
        f"\ne2e without rel:  {e2e_plain_state_dict} \n {calculate_f1(e2e_plain_state_dict, 'e2e_plain')}")
...
def unirel_span_extractor(
    ...
    assert isinstance(head_preds, np.ndarray)
    assert isinstance(tail_preds, np.ndarray)
    assert isinstance(span_preds, np.ndarray)
    ...
    # print(calclulate_f1(state_dict))
    all_metric_results = calculate_f1(state_dict, 'all')
    print(f"\nall:  {state_dict} \n {calculate_f1(state_dict, 'all')}")
    with open(path, "w") as wp:
        json.dump(extract_data, wp, indent=2, ensure_ascii=False)
    return all_metric_results["all-prec"], all_metric_results["all-recall"], all_metric_results["all-f1"]
